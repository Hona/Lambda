...
namespace LambdaUI.Services
{
    internal static class TempusUpdaterService
    {
        ...
        private static string FormatCourseRecords(IEnumerable<CourseWr> records) => records.Aggregate("",
            (currentString, nextItem) => currentString +
                                         $"{nextItem.RecordInfo.ClassString()} C{nextItem.ZoneInfo.Zoneindex} | [{nextItem.MapInfo.Name.EscapeDiscordChars()}](https://tempus.xyz/maps/{nextItem.MapInfo.Name}) | [**{FormattedDuration(nextItem.RecordInfo.Duration).EscapeDiscordChars()}**](https://tempus.xyz/records/{nextItem.RecordInfo.Id}) | [{nextItem.PlayerInfo.Name.EscapeDiscordChars()}](https://tempus.xyz/players/{nextItem.PlayerInfo.Id})" +
                                         Environment.NewLine);
        private static string FormatBonusRecords(IEnumerable<BonusWr> records) => records.Aggregate("",
            (currentString, nextItem) => currentString +
                                         $"{nextItem.RecordInfo.ClassString()} B{nextItem.ZoneInfo.Zoneindex} | [{nextItem.MapInfo.Name.EscapeDiscordChars()}](https://tempus.xyz/maps/{nextItem.MapInfo.Name}) | [**{FormattedDuration(nextItem.RecordInfo.Duration).EscapeDiscordChars()}**](https://tempus.xyz/records/{nextItem.RecordInfo.Id}) | [{nextItem.PlayerInfo.Name.EscapeDiscordChars()}](https://tempus.xyz/players/{nextItem.PlayerInfo.Id})" +
                                         Environment.NewLine);
    }
}
